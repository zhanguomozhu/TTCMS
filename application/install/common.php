...
function check_mysql()
{
	...
	$db_host = $_GET['db_address']; //数据库地址
	$db_port = $_GET['db_port'];	//数据库端口
	$db_name = $_GET['db_name'];	//数据库名称
	$db_user = $_GET['db_user'];	//用户名
	$db_pwd  = $_GET['db_pwd'];		//密码
			...
			switch($prefix)
			{
				...
				default :
				{
					if($flage)
					{
						...
						if(substr($line,-1)==";")
						{
							$sql = str_replace('tp_',$db_pre,$sql);
							$sqls[]=$sql;
							$sql="";
						}
					}
...
function install_sql(){
	...
	//检测SQL安装文件
	$sql_file = EXTEND_PATH.'/install/sql/table.sql';
	...
	//安装地区数据
	$sql_data_file = EXTEND_PATH.'/install/sql/data.sql';
...
EOF;
	...
	//插入管理员数据
	$adminSql = 'insert into `'.$db_pre.'admin` (`username`,`password`,`admin`) values ("'.$admin_user.'","'.md5($admin_pwd).'",1)';
	...
	//插入站点数据
	$adminSql = 'replace into `'.$db_pre.'conf` VALUES
	(null,\'1\',\'站点名称\',\'site_name\',\'1\',\''.$site_name.'\'),
	(null,\'1\',\'站点关键词\',\'site_keywords\',\'2\',\''.$site_keywords.'\'),
	(null,\'1\',\'站点描述\',\'site_description\',\'2\',\''.$site_description.'\')';
	if(!db_query($adminSql,$mysql_link))
	{
	...
	$url = 'http://'.$_SERVER['SERVER_NAME'].':'.$_SERVER["SERVER_PORT"].url('install/index/clear_cache');
