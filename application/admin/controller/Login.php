...
class Login extends Controller
{
	  	...
	  	if(request()->isAjax())
	  	{
			...
			{//账号
	    		if(model('admin')->where(['username'=>$username])->find()){
		        	return show(1,'管理员账号正确');
			  	}else{
			   		return show(0,'管理员账号错误');
			  	}
			...
			{//邮箱验证码
	    		...
	    		if(md5($email . $email_code) != session($email.'_email_code')){
	    			return show(0,'邮箱验证码错误');
			  	}else{
			  		//验证成功删除session
			  		session($email.'_email_code', null);
			  		return show(1,'邮箱验证码正确');
			  	}
			}
		}
	}
	...
	public function register()
	{
		if(request()->isPost()){
			...
			if(model('Admin')->register(true)){
				return show(1,'注册成功',[],url('admin/Login/login'));
			}else{
				return show(0,'注册失败,请重试');
			};
			return;
		}
		return $this->fetch();
	...
	public function yanzheng(){
	  	if(request()->isAjax())
	  	{
			...
			{
	    		if(model('admin')->where(['username'=>$username])->find()){
		        	return show(0,'管理员账号已存在');
			  	}else{
			   		return show(1,'管理员账号可以使用');
			  	}
			}elseif($phone && !$code)
			{
